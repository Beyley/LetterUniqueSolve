...
pub fn loadWordsRu(allocator: std.mem.Allocator, path: []const u8, word_length: usize) ![]const []const u8 {
    ...
    while (lines_iter.next()) |line| {
        ...
        //     {
        //         var codepoint_iter = std.unicode.Utf8Iterator{ .bytes = trimmed, .i = 0 };
        //         var skip = false;
...
pub fn loadWordsEo(allocator: std.mem.Allocator, path: []const u8, word_length: usize) ![]const []const u8 {
    var file = try std.fs.cwd().openFile(path, .{});
    defer file.close();
